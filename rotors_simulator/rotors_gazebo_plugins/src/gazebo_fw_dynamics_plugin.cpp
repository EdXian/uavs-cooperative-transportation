...
void GazeboFwDynamicsPlugin::UpdateForcesAndMoments() {
  ...
  math::Quaterniond W_rot_W_B = link_->WorldPose().Rot();
  math::Vector3d B_air_speed_W_B = W_rot_W_B.RotateVectorReverse(
      link_->WorldLinearVel() - W_wind_speed_W_B_);
  math::Vector3d B_angular_velocity_W_B = link_->RelativeAngularVel();
  ...
  const math::Vector3d forces =
      math::Vector3d (forces_B[0], -forces_B[1], -forces_B[2]);
  const math::Vector3d moments =
      math::Vector3d (moments_B[0], -moments_B[1], -moments_B[2]);
